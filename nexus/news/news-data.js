/* YEHAVHA NEWS canonical article registry.
   Add each published article here once. All news lists, category views,
   search results and archives are derived from this single source. */
window.YEHAVHA_NEWS_CONFIG = Object.freeze({
  categories: ["정치", "정부", "경제·산업", "사회", "법·정책", "AI·기술"],
  resultsPageSize: 20,
  homeLatestLimit: 10,
  categoryLatestLimit: 3
});

window.YEHAVHA_NEWS_DATA = Object.freeze([
  {
    id: "2026-09-15-nhrck-defense-rights-repeal-agenda",
    date: "2026-09-15",
    category: "정치",
    author: "이명훈",
    type: "기사",
    title: "인권위, 윤 전 대통령 방어권 권고 폐기안 정식 상정…결론은 아직",
    summary: "국가인권위원회가 지난해 방어권 보장 권고를 폐기하고 사과하자는 안건을 정식 상정했다. 현재 단계는 상정이며 폐기 자체가 최종 의결된 것은 아니다.",
    href: "./articles/2026-09-15-nhrck-defense-rights-repeal-agenda.html",
    keywords: "국가인권위원회 윤석열 방어권 권고 폐기 대국민 사과 전원위원회"
  },
  {
    id: "2026-09-15-korea-adb-critical-minerals-energy",
    date: "2026-09-15",
    category: "정부",
    author: "이명훈",
    type: "기사",
    title: "정부, ADB와 핵심광물·AI·원전 협력 확대…중앙아 사업 발굴 추진",
    summary: "한국의 산업기술과 ADB의 개발금융을 결합해 중앙아시아 핵심광물·AI·에너지 프로젝트를 실제 사업으로 전환하는 협력이 추진된다.",
    href: "./articles/2026-09-15-korea-adb-critical-minerals-energy.html",
    keywords: "정부 ADB 중앙아시아 핵심광물 원전 에너지 AI 공급망 개발금융"
  },
  {
    id: "2026-09-15-kospi-ai-mideast-selloff",
    date: "2026-09-15",
    category: "경제·산업",
    author: "이명훈",
    type: "기사",
    title: "코스피 3.26% 급락…중동 불안과 AI 투자 경계가 동시에 흔들었다",
    summary: "코스피가 14일 6,684.37로 3.26% 하락했다. 중동 위험, 유가와 금리 부담, 글로벌 AI 관련주 조정이 동시에 시장을 압박했다.",
    href: "./articles/2026-09-15-kospi-ai-mideast-selloff.html",
    keywords: "코스피 중동 유가 금리 AI 기술주 외국인 기관 환율 증시"
  },
  {
    id: "2026-09-15-college-application-system-relief",
    date: "2026-09-15",
    category: "사회",
    author: "이명훈",
    type: "기사",
    title: "대입 수시 원서접수 장애 후속조치…미접수 수험생 구제절차 마련",
    summary: "교육부와 대교협이 시스템 장애로 정상 접수를 마치지 못한 수험생의 구제기준을 마련하고 원서접수 체계 전반을 개선하기로 했다.",
    href: "./articles/2026-09-15-college-application-system-relief.html",
    keywords: "수시 원서접수 장애 대교협 교육부 수험생 구제 대학 입시 교육"
  },
  {
    id: "2026-09-15-itaewon-special-act-extension",
    date: "2026-09-15",
    category: "법·정책",
    author: "이명훈",
    type: "기사",
    title: "이태원참사 특별법 개정 오늘 시행…특조위 활동·피해자 신청기간 연장",
    summary: "특별법 개정안이 15일 공포·시행돼 특조위 활동이 2027년 9월까지 연장되고 피해자 인정·치유휴직 신청기간도 늘어난다.",
    href: "./articles/2026-09-15-itaewon-special-act-extension.html",
    keywords: "이태원참사 특별법 특조위 피해자 인정 치유휴직 시행 행정안전부"
  },
  {
    id: "2026-09-15-korea-central-asia-ai-science-ministers",
    date: "2026-09-15",
    category: "AI·기술",
    author: "이명훈",
    type: "기사",
    title: "한·중앙아 5개국 첫 AI·과학기술 장관회의…협력축 넓어진다",
    summary: "한국과 중앙아시아 5개국이 처음으로 AI·과학기술 장관급 회의를 열고 AI, 과학기술, 기후기술 실증 협력 의제를 논의했다.",
    href: "./articles/2026-09-15-korea-central-asia-ai-science-ministers.html",
    keywords: "한국 중앙아시아 AI 과학기술 장관회의 기후기술 GGGI 인공지능"
  },
  {
    id: "2026-09-14-foreign-espionage-law",
    date: "2026-09-14",
    category: "법·정책",
    author: "이명훈",
    type: "기사",
    title: "형법에 ‘외국 등을 위한 간첩’ 신설…국가기밀 보호 범위 넓어졌다",
    summary: "외국 등을 위한 국가기밀 탐지·수집·누설 등을 별도로 처벌하는 형법 조항이 시행되면서 외국 연계 기밀침해에 대한 형사법상 대응 범위가 넓어졌다.",
    href: "./articles/2026-09-14-foreign-espionage-law.html",
    keywords: "형법 간첩 외국 국가기밀 산업기술 기술유출 형사법"
  },
  {
    id: "2026-09-14-fengzhen-min-isds-win",
    date: "2026-09-14",
    category: "법·정책",
    author: "이명훈",
    type: "기사",
    title: "정부, 중국 투자자 ISDS 취소절차 승소…원 중재 승소 판정 확정",
    summary: "중국 투자자가 한국 정부를 상대로 제기한 국제투자분쟁의 판정 취소신청이 전부 기각되면서 기존 중재 승소 판정이 확정됐다.",
    href: "./articles/2026-09-14-fengzhen-min-isds-win.html",
    keywords: "정부 중국 투자자 ISDS 국제투자분쟁 ICSID 중재 법무부"
  },
  {
    id: "2026-09-14-supreme-court-nominee-hearing",
    date: "2026-09-14",
    category: "정치",
    author: "이명훈",
    type: "기사",
    title: "김성수 대법관 후보자 청문회 오늘 개최…공석 해소 절차 분기점",
    summary: "김성수 대법관 후보자에 대한 국회 인사청문회가 열리면서 대법원 공석 해소를 위한 임명 절차가 주요 분기점을 맞았다.",
    href: "./articles/2026-09-14-supreme-court-nominee-hearing.html",
    keywords: "김성수 대법관 후보자 국회 인사청문회 대법원 공석 정치"
  },
  {
    id: "2026-09-13-saudi-energy-route-risk",
    date: "2026-09-13",
    category: "경제·산업",
    author: "이명훈",
    type: "기사",
    title: "사우디 동서 송유관 일시 폐쇄…중동 에너지 우회망까지 흔들린다",
    summary: "사우디 동서 송유관의 일시 폐쇄로 호르무즈 해협을 우회하는 핵심 에너지 수송망의 안정성까지 중동 위험의 영향권에 들어왔다.",
    href: "./articles/2026-09-13-saudi-energy-route-risk.html",
    keywords: "사우디 동서 송유관 중동 에너지 원유 홍해 호르무즈 공급망"
  },
  {
    id: "2026-09-13-fema-workforce-ruling",
    date: "2026-09-13",
    category: "정부",
    author: "이명훈",
    type: "기사",
    title: "미 연방법원, FEMA 인력 절반 감축 추진에 위법 판단",
    summary: "미 연방법원이 연방재난관리청 FEMA 인력을 대폭 감축하려던 추진 과정에 위법성이 있다고 판단하면서 재난대응 조직 운영을 둘러싼 법적 쟁점이 부각됐다.",
    href: "./articles/2026-09-13-fema-workforce-ruling.html",
    keywords: "미국 연방법원 FEMA 연방재난관리청 인력 감축 재난 대응 정부"
  }
]);
