(() => {
  function init() {
    ...
    if (!grid || !episodes.length) return;
    if (seriesCount) seriesCount.textContent = `${episodes.length} CARDS`;

    function openViewer(item) {
      if (!viewer || !viewerImage || !viewerIndex || !viewerTitle || !viewerTakeaway || !viewerKeywords) return;
