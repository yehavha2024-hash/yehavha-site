...
function parseRss(xml, provider, group) {
  ...
  for(const block of blocks.slice(0,12)){const title=tag(block,'title');const link=tag(block,'link');const description=tag(block,'description');const pubDate=tag(block,'pubDate');if(title&&link)items.push({title,url:link,snippet:description,publishedAt:pubDate||null,provider,group});}
  return items;
}
...
function annotateEvidence(type,items){return items.map((item,index)=>{const text=`${item.title} ${item.snippet}`;return{id:`S${String(index+1).padStart(2,'0')}`,...item,host:sourceHost(item.url),topics:topicMatches(type,text),sentiment:scoreSentiment(text).direction,highRisk:highRisk(text)};});}
...
function reportSummary(target,signals,evidence){if(!evidence.length)return `${target}에 대해 자동으로 확인된 공개 출처가 충분하지 않습니다. 원문 출처를 추가 확인한 뒤 판단해야 합니다.`;const strong=signals.filter(s=>s.independentSources>=2).slice(0,3);if(!strong.length)return `${target}에 관한 공개자료는 확인됐지만 동일 주제가 복수의 독립 출처에서 충분히 반복되지는 않았습니다. 단일 후기나 단일 기사만으로 일반화하지 않는 것이 적절합니다.`;const parts=strong.map(s=>s.state==='conflicted'?`${s.topic}에 대해서는 긍정·부정 평가가 함께 나타납니다`:s.state==='negative'?`${s.topic} 관련 부정적 신호가 복수 출처에서 확인됩니다`:s.state==='positive'?`${s.topic} 관련 긍정적 신호가 복수 출처에서 확인됩니다`:`${s.topic} 관련 언급이 반복됩니다`);return `${target}에 대한 공개자료를 교차 확인한 결과, ${parts.join('. ')}. 아래 출처별 근거와 상반된 자료를 함께 확인해야 합니다.`;}
