(() => {
  ...
  function ensureSiteFooterDisclosure() {
    ...
    let notice = footerMeta.querySelector('.ai-disclosure');
    if (!notice) {
      ...
      const topLink = footerMeta.querySelector('.top-link');
      if (contact) contact.insertAdjacentElement('afterend', notice);
      else if (topLink) topLink.insertAdjacentElement('beforebegin', notice);
      else footerMeta.appendChild(notice);
    } else {
      notice.textContent = aiNotice;
    }
