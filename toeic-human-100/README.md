...
- 고등학교·수능 코퍼스 기반 필수어휘 1,000개를 별도 빈도·우선순위 참고층으로 보강
- 핵심 활성어휘 목표: 약 2,520개
- 숙어·연어: 하루 최소 6개
- 문법: 하루 최소 3개
...
2. `해부·학습`
   ...
   - 고등학교·수능 코퍼스에서 반복되는 추상어휘·학술어휘·논리어휘를 우선순위 참고자료로 활용
...
여기에 `고등학생 필수 어휘 1000 (빈도순)` 분석자료를 별도 참고층으로 추가했습니다. 원자료는 영어 지문 코퍼스 약 20만 단어를 기반으로 CEFR B1 이상 어휘 가운데 초·중학 기초어휘와 구체 명사를 제외한 1,000개 표제어를 순위·빈도·한국어 뜻으로 제시합니다.

- `high-school-csat-1000.csv`: 1~1000위 표제어와 코퍼스 빈도
- `high-school-csat-1000-meanings.csv`: 원자료의 한국어 뜻
- 역할: 고등학교·수능형 비문학 독해에서 중요한 어휘의 우선 학습·재노출 판단 보강
- 기존 `master-lexicon-v2.json` 4,786개와 분리 관리
- 수능 보강층 1,000개를 본문에 억지로 전부 삽입하지 않음
- 기존 마스터와 겹치는 단어는 TOEIC 역할 태그와 수능 코퍼스 빈도를 서로 다른 신호로 활용
- 제공자료의 철자·표제어·뜻은 원자료 층에서 우선 보존하고 정규화·뜻 검수는 별도 단계로 수행
...
- 고등학교·수능 1,000개 보강층은 기존 선별축을 대체하지 않고 교집합·빈도·재노출 우선순위 판단을 보조
...
- `high-school-csat-1000.csv` — 고등학교·수능 코퍼스 1,000개 순위·빈도 원자료층
- `high-school-csat-1000-meanings.csv` — 고등학교·수능 코퍼스 1,000개 한국어 뜻 원자료층
- `MASTER_CORPUS_SOURCES.md` — 각 어휘층의 출처·역할·통합 원칙
- `coverage-policy-v2.json` — 집중독해·활성어휘·숙어·문법·참고어휘층 커버리지 정책
...
검증 기준은 `500~650단어 집중독해 + 핵심 활성어휘 + 숙어·문법 분리학습`이며, 전체 마스터 어휘나 고등학교·수능 보강층 1,000개를 본문에 강제 삽입하지 않습니다. 수능 보강층은 별도 교집합·미포함·빈도 우선순위 감사 대상으로 관리합니다.

## 저작권 원칙

TOEIC·TEPS 공식 기출문장을 대량 복제하지 않습니다. 시험의 일반적인 평가영역·독해유형·문서형식을 학습 목적으로 참고하고, 본문과 문항은 자체 제작·생성한 학습자료를 사용합니다.

고등학교·수능 보강자료 역시 문항·지문을 복제하는 용도가 아니라 제공된 어휘 통계와 뜻을 독립적인 참고 데이터층으로 관리합니다.
