/* DAY 011~100 generated-study normalization.
   Owns scaffold removal, active-vocabulary selection and phrase enrichment.
   Reading length is owned only by reading-length-normalizer.js. */
(function (root) {
  ...
  function buildActivePlan(result) {
    ...
    const activePool = uniqueEntries(scheduled)
      .sort((a, b) => priority(a) - priority(b)
        || (firstDay.get(lemmaOf(a).toLowerCase()) || 100) - (firstDay.get(lemmaOf(b).toLowerCase()) || 100)
        || lemmaOf(a).localeCompare(lemmaOf(b)))
      .slice(0, ACTIVE_UNIQUE_TARGET)
      .sort((a, b) => (firstDay.get(lemmaOf(a).toLowerCase()) || 100) - (firstDay.get(lemmaOf(b).toLowerCase()) || 100)
  ...
  function normalizeDay(day, plan) {
    ...
    day.reading.paragraphs = day.reading.paragraphs.map(stripScaffolding).filter(Boolean);
  ...
  builder.build = master => normalizeResult(originalBuild(master));
  builder.normalizeStudyPlan = normalizeResult;

  if (root.TOEIC_READING_V2_READY && typeof root.TOEIC_READING_V2_READY.then === 'function') {
    root.TOEIC_READING_V2_READY = root.TOEIC_READING_V2_READY.then(normalizeResult);
  }
})(globalThis);
