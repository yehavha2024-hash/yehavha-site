...
for (const item of (master.entries || [])) {
  ...
  auditEntries.push({id:item.id,lemma:item.lemma,sourceLists:item.sourceLists,roles:item.roles,requiredExposure:required,exposureCount:total,meetsMinimum:total>=required,exposureDays:days.map(d=>d.day),dayCounts:days,seenByDay80:days.some(d=>d.day<=80)});
}
...
const report = {
  ...
  staged:!finished,
  dayCount,expectedDays,tepsDayCount,
  masterHeadwords:auditEntries.length,
  seenHeadwords:present.length,
  missingHeadwords:missing.length,
  missingByDay80:missingBy80.length,
  belowMinimumExposure:belowMinimum.length,
