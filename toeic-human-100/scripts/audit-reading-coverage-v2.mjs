...
const CONTENT_FILE = path.join(DIR, 'reading-content-v2.js');
...
if (!fs.existsSync(CONTENT_FILE)) throw new Error('reading-content-v2.js missing');
...
vm.runInContext(`${source}\nglobalThis.__READING_V2__ = TOEIC_READING_V2;`, context, { filename: CONTENT_FILE });
...
for (const day of (program.days || [])) {
  ...
  const practice = [
    ...
    ...(day.practice?.part6 || []),
    ...(day.practice?.part7 || [])
  ].flatMap(q => [q.question, ...(q.options || [])]).join(' ');
  const analysis = [
    ...(day.sentenceLab || []).map(x => x.sentence),
    ...(day.sentenceStructures || []).map(x => x.example)
  ].join(' ');
  dayTexts.set(day.day, normalizeText(`${reading} ${practice} ${analysis}`));
}
...
function occurrences(text, form) {
  ...
  if (!cleanForm.includes(' ')) {
    ...
    let count = 0;
    for (const token of tokens) if (token === cleanForm) count += 1;
    return count;
  }
...
for (const role of ['general-core','toeic-specific','academic-book-extension']) {
  ...
  byRole[role] = {
    total: items.length,
    seen: seen.length,
    missing: items.length - seen.length,
    coveragePercent: items.length ? Number((seen.length / items.length * 100).toFixed(2)) : 0
  };
}

const dayCount = program.days?.length || 0;
const finished = dayCount === (program.meta?.totalDays || 100);
const report = {
  generatedAt: new Date().toISOString(),
  staged: !finished,
  dayCount,
  expectedDays: program.meta?.totalDays || 100,
  ...
  byRole,
  repetition: {
    seenAtLeast2: auditEntries.filter(x => x.exposureCount >= 2).length,
    seenAtLeast4: auditEntries.filter(x => x.exposureCount >= 4).length,
    seenAtLeast8: auditEntries.filter(x => x.exposureCount >= 8).length
  },
...
const md = `# Reading V2 Master Coverage\n\n` +
  ...
  `- V2 작성일수: ${dayCount}/${report.expectedDays}\n` +
  ...
  `\n\n## 판정\n\n${finished ? (missing.length ? '100일 작성은 완료됐으나 미등장 마스터 어휘가 남아 있으므로 콘텐츠 완성으로 판정하지 않는다.' : '마스터 headword 1회 이상 노출 기준을 충족했다.') : '현재는 단계적 제작 중이므로 미등장 항목은 오류가 아니라 다음 DAY 배치대상이다.'}\n`;
