(function(){
  ...
  function ensureDocumentAiDisclosure(root=document){
    root.querySelectorAll?.('.document-footer-copy').forEach(copy=>{
      if(copy.querySelector('.ai-disclosure'))return;
      const contact=copy.querySelector('a[href^="mailto:"]')?.closest('p');
      if(!contact)return;
      const p=document.createElement('p');
      p.className='ai-disclosure';
      p.textContent=aiNotice;
      contact.insertAdjacentElement('afterend',p);
    });
  }
