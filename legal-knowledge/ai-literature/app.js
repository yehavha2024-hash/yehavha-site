(() => {
  ...
  const state = { query:"", type:"전체", area:"전체", role:"전체", priority:"all", stage:"all", routeIds:null };
  ...
  function renderFilters() {
    ...
    const roleOrder = ["직접 인용 핵심문헌", "반대학설", "비교법", "최신문헌"];
    const presentRoles = new Set(records.flatMap(r => r.evidenceRoles || []));
    const roles = ["전체", ...roleOrder.filter(role => presentRoles.has(role))];
    ...
    els.roleFilters.replaceChildren(...roles.map(role => makeButton(role, "role", role)));

    [els.typeFilters, els.areaFilters, els.roleFilters].forEach(container => {
      container.querySelectorAll("button").forEach(button => {
        const key = button.dataset.filterKey;
        button.classList.toggle("active", button.dataset.filterValue === state[key]);
      });
    });
  }
  ...
  function renderRoutes() {
    els.routeGrid.replaceChildren(...routes.map((route, index) => {
      ...
      button.addEventListener("click", () => {
        ...
        state.query = ""; state.type = "전체"; state.area = "전체"; state.role = "전체"; state.priority = "all"; state.stage = "all";
        els.searchInput.value = ""; els.prioritySelect.value = "all"; els.stageSelect.value = "all";
  ...
  function searchableText(record) {
    return normalize([
      ...
      ...(record.legalAreas || []), ...(record.issues || []), ...(record.evidenceRoles || []),
      ...(record.mustRead || []), ...(record.argumentUse || [])
    ].join(" "));
  }
  ...
  function filteredRecords() {
    ...
    return records.filter(record => {
      ...
      if (state.role !== "전체" && !(record.evidenceRoles || []).includes(state.role)) return false;
  ...
  function roleTags(record, limit = 4) {
    return (record.evidenceRoles || []).slice(0, limit).map(role => `<span class="tag">${escapeHtml(role)}</span>`).join("");
  }
  ...
  function cardFor(record) {
    ...
    article.innerHTML = `
      ...
      <div class="tag-row">${roleTags(record)}</div>
      <div class="card-use"><strong>연구 접목</strong>${escapeHtml(record.researchFit)}</div>
    `;
    const actions = document.createElement("div");
    actions.className = "card-actions";
    const detail = document.createElement("button");
    detail.type = "button"; detail.className = "card-button"; detail.textContent = "연구노트 열기 →";
    detail.addEventListener("click", () => openDetail(record));
    actions.appendChild(detail); article.appendChild(actions);
    return article;
  }
  ...
  function renderCards() {
    ...
    const filters = [];
    if (state.routeIds) filters.push("읽기 경로");
    if (state.role !== "전체") filters.push(state.role);
    els.resultCount.textContent = `${result.length}개 문헌 표시${filters.length ? ` · ${filters.join(" · ")}` : ""}`;
    els.emptyState.hidden = result.length !== 0;
  }

  function relatedList(record) { return (record.related || []).map(id => byId.get(id)).filter(Boolean); }
  ...
  function openDetail(record) {
    ...
    els.detailContent.innerHTML = `
      ...
      <section class="detail-section" id="detail-use"><h3>05 주장에 쓰는 방식</h3>${listHtml(record.argumentUse)}</section>
      <section class="detail-section" id="detail-fit"><h3>06 현재 연구와의 구체적 접목</h3><p>${escapeHtml(record.researchFit)}</p></section>
      <section class="detail-section" id="detail-counter"><h3>07 반대논리·적용상 주의</h3><p>${escapeHtml(record.counterpoint)}</p></section>
      <section class="detail-section" id="detail-related"><h3>08 반대·보완·연결 문헌</h3>${relatedHtml}</section>
      <section class="detail-section" id="detail-source"><h3>09 원문·서지 링크</h3><p>${escapeHtml(record.access)}</p><a class="source-link" href="${escapeHtml(record.url)}" target="_blank" rel="noopener noreferrer">원문·서지 확인 ↗</a></section>
      <footer class="detail-footer"><p>Copyright © 이명훈 2026. All rights reserved.</p><p>AI 활용 요약은 원문을 대체하지 않으며 직접 인용 전 원문·쪽수·최신 규범·판례를 재검증합니다.</p></footer>
    `;
    els.detailContent.querySelectorAll(".inline-related").forEach(button => {
      button.addEventListener("click", () => { const target = byId.get(button.dataset.relatedId); if (target) openDetail(target); });
    });
    els.dialog.showModal(); document.body.style.overflow = "hidden"; els.dialog.querySelector(".dialog-shell").scrollTop = 0;
  }

  function closeDialog() { if (els.dialog.open) els.dialog.close(); document.body.style.overflow = ""; }
  function resetFilters() {
    state.query = ""; state.type = "전체"; state.area = "전체"; state.role = "전체"; state.priority = "all"; state.stage = "all"; state.routeIds = null;
    els.searchInput.value = ""; els.prioritySelect.value = "all"; els.stageSelect.value = "all";
    renderFilters(); renderCards();
  }
  ...
  const domestic = records.filter(r => r.type === "국내 박사학위논문" || r.type === "국내 학술논문").length;
  const overseas = records.filter(r => r.language === "영어").length;
  els.recordCount.textContent = `코어 문헌 ${records.length}`;
  els.domesticCount.textContent = `국내 박사·학술 ${domestic}`;
  els.overseasCount.textContent = `해외 핵심 ${overseas}`;
  els.routeCount.textContent = `읽기 경로 ${routes.length}`;
  initStageOptions(); renderFilters(); renderRoutes(); renderCards();
})();
